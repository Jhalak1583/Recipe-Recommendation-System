...
const savedRecipes = Object.keys(window.localStorage).filter((item) => {
  return item.startsWith("cookwise-recipe");
});
...
if (savedRecipes.length) {
  savedRecipes.map((savedRecipe, index) => {
    ...
    const isSaved = window.localStorage.getItem(`cookwise-recipe${recipeId}`);
  ...
  $savedRecipeContainer.innerHTML += `<p class="body-large">You have not saved any recipes yet</p>`;
}
